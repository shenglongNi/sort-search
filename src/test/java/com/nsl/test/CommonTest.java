...
public class CommonTest {
    ...
    public void genTreeDataTest2() {
        ...
        List<SysMenu> sysMenuList = JSONObject.parseObject(rawData, new TypeToken<List<SysMenu>>(){}.getType());


        List<SysMenu> treeData = sysMenuList.stream()
                .filter(item -> item.getParentId() == sysMenuList.stream()
                        .map(SysMenu::getParentId)
                        .min(Comparator.comparing(parentId -> parentId))
                        .get())
                .map(item -> convert(item, sysMenuList))
    ...
    private SysMenu convert(SysMenu parent, List<SysMenu> sysMenus) {
        for (SysMenu sysMenu : sysMenus) {
            if (sysMenu.getParentId().equals(parent.getId()) ) {
                parent.getChildren().add(sysMenu);
                parent.getChildren().sort(Comparator.comparing(SysMenu::getOrderNum));
                convert(sysMenu, sysMenus);
            }
        }
        return parent;
    }
